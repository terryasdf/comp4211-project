...
prompt_loc = {
    "zero-shot": f"{PROMPT_PATH}/task2.txt",
}
...
class GPT_QUERY:
    ...
    def gpt(self, x, prompt, processor=None, cot=False):
        message = {
            "role": "user",
            "content": prompt + x + "\nAnswer:" +
    ...
    def predict(self, prompt_type, processor, vote=1, cot=False):
        try:
            ...
            with tqdm(total=set_size) as bar:
                for index_A, row_A in table.head(set_size).iterrows():
                    ...
                    try:
                        table.at[index_A, 'predict'] = self.gpt(joined_sentence, prompt, processor, cot)
                    except Exception as error:
                        print(f"An error occurred at index {index_A}:", error)
                        table.at[index_A, 'predict'] = "ERR"
                    bar.update(1)
        finally:
            table.head(set_size)[['predict', 'relation',]].to_csv(f"results/task2_{prompt_type}_cot_{cot}.csv")
...
def run_predict(type: str, cot=False, processor=categorical_clean):
    gpt_client = GPT_QUERY(test.shape[0], test)
    gpt_client.predict(type, vote=3, processor=processor, cot=cot)
    return gpt_client.estimate()

run_predict("zero-shot", processor=None)
